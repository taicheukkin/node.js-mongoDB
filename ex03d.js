...
cookie.maxAge means:
- The browser will keep the session cookie for 30 minutes after it was last set.
- As long as the cookie is still valid, refreshing the page (F5) will send the same session ID back 
to the server, and req.session.user will still be available.
- Once the 1 minute expire, the cookie is no longer sent, so the server won’t recognize the user 
as logged in anymore.

👉 So in your current setup:
If the user is idle for 1 minute and then presses F5, the session will have expired and they’ll be logged out.
...
future, so the session stays alive as long as the user is active. pls check ex03d2.js
*/
